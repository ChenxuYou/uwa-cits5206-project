...
| R11 | **.NET 10 immaturity** — a dependency or SDK regression blocks the build | Low | Medium | SDK version pinned in CI; minimal dependency surface; lockfile committed | A build failing on the toolchain rather than on our code | Wenmin Luo |
| R12 | **IP or licensing dispute** over a jointly-owned deliverable | Low | Medium | Ownership position confirmed in writing by the client on 20 August; `NOTICE` grants UWA perpetual internal-use permission and reserves portfolio rights for each author; no open-source licence granted until handover, because one joint owner cannot grant one alone | Any request to relicense or redistribute before handover | Chenxu You |
