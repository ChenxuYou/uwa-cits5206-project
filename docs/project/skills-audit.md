...
| G2 | **Deployment and operations** — no member has taken an application to a running server with TLS, DNS and a rollback path | A `localhost` demo is not a handover. Criterion 3 names software deployment explicitly | Deployment is a milestone with owners and dates, not an end-of-semester task. Hosting decided 9 Sep; the deploy sprint runs the week of 28 Sep; staging live 2 Oct (**M5**); the client uses it unaccompanied for the eleven days before the final release | Dai Lam La La |
| G3 | **Translating a business method into a workflow** | This is where projects of this shape fail: a tool that is confidently wrong | Written source-precedence rule; every requirement carries a `[C]`/`[G]`/`[W]`/`[K]` marker; one owner for the guide-vs-calculator reconciliation instead of five parallel opinions | Wenmin Luo |
