...
| M3 | ✅ **Rates, proposed rates and balance** | **18 Sep 2026** | Three rates per capability with the figures behind each; proposed rates and the resulting surplus or deficit. **Met 22 Sep, four days late.** US-09, US-11 and US-12 landed in #73 and #74; an audit against [`user-stories.md`](../spec/user-stories.md) found US-10 and US-13 short — the platform step could not be reopened, going back could lose what was typed, required explanations were not checked again at submission, and the sealed record did not carry the explanations beside their figures. See change log 1.7 |
...
| Version | Date | Change |
| --- | --- | --- |
| 1.7 | 22 Sep 2026 | **M3's open criteria completed**, on top of 1.6. **US-10:** the platform step reopens an existing cycle — rename or add capabilities, change the name, and the period and unit while no figure is yet expressed in them (the page says why when they are locked); the capacity step saves before going back, as the rates step already did; and a step-bar link followed with unsaved changes saves first on those two steps and warns on the others. **US-13:** the costs step gains costing assumptions for the section as a whole; the review shows the guide's Step 5 checklist; the three required explanations (utilisation, rate variance, deficit) are checked again at submission from one rule, because a cost added after the rates step could leave a deficit nobody was asked about; and the sealed snapshot moves to **schema 1.3**, carrying the costing assumptions and how each capability's capacity was built, with every note. The PDF now prints every cost and funding line with its note or justification, and the capacity section with the utilisation assumptions and each deduction's note, ahead of the rates they feed. Records sealed under 1.1 and 1.2 still render as sealed. Also fixed: after approving or returning a record, the approver was redirected to the custodian's review page and shown "access denied" |
