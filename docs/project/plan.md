...
**Version:** 1.10 — 24 September 2026
**Owner:** Chenxu You
...
| Version | Date | Change |
| --- | --- | --- |
| 1.10 | 24 Sep 2026 | **Minutes caught up and `docs/meetings/` reorganised.** Six missing records written up: facilitator checkpoints of 5 Aug, 19 Aug, 16 Sep and 23 Sep, the team meeting of 22 Aug, and the client meeting of 22 Sep, where the client said she would ask UWA IT for a domain and resources and that the team may deploy staging itself in the meantime. Meeting records now sit in `client/`, `facilitator/` and `team/` under one naming rule, `YYYY-MM-DD-<type>-meeting.md`, with an index; links across the repository follow the moves. §5's hosting row is not yet updated, and is an action in the 22 Sep minutes |
