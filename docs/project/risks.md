...
| R3 | The calculator's arithmetic is reproduced where it departs from the guide | Low | High | Client confirmed in writing on 20 Aug 2026 that **the guide governs**; fixtures assert the guide's figure and record both where they differ, so a future reader cannot mistake a deliberate divergence for a bug of ours. Divergences are reported to the client as we meet them | Any fixture asserting a figure taken from the calculator without a note | Wenmin Luo |
...
| R14 | Seeded demo credentials reach a deployed instance | Medium | High | `entry`/`approver` demo accounts exist for local development only and are named as such in `src/README.md`. They are seeded in Development only; a staging database starts with one bootstrap administrator from the server's environment, and that person — like anyone whose password an administrator set — must choose a new password at first sign-in | Any deployment attempted with the seeded accounts present | Jaswanth Vericherla |
| R15 | A sealed record is altered at the database level | Low | High | One-way seal transition; a concurrency stamp on every cycle, so a request that read it before the seal cannot write afterwards (added 25 Sep after the audit showed a double-clicked approval could rewrite the snapshot and hash); no route accepts a write to a sealed record; immutable JSON snapshot with a **SHA-256 integrity hash**; recomputation on read forbidden — a verification job may compare and raise an alarm, never correct; append-only audit entries | Any hash mismatch on verification | Chenxu You |
| R16 | Transport or session weakness | Low | Medium | HTTPS only with HSTS, served by Caddy with the application on loopback ([`deploy/`](../../deploy/README.md)); anti-framing headers; HTTP-only cookie, Secure outside development, with a 2-hour expiry and sliding renewal; PBKDF2 password hashing at 210,000 iterations with rehash on sign-in; five failed attempts lock an account for 15 minutes, and one address may try ten times a minute; a security stamp rotated on password change ends older sessions — no custom cryptography | Any HTTP endpoint reachable in staging | Dai Lam La La (transport), Jaswanth Vericherla (session) |
| R17 | A dependency vulnerability | Low | Medium | Minimal dependency surface (one direct NuGet package today); automated vulnerability scanning on every push and pull request. High and Critical advisories fail the build unless assessed and recorded in [`.github/known-advisories.txt`](../../.github/known-advisories.txt) with a reason and a review date | Any scan finding a High or Critical advisory that is not already assessed | Dai Lam La La |
| R17a | **Live and accepted: CVE-2025-6965** (GHSA-2m69-gcr7-jv3q, High, CVSS 7.2). `SQLitePCLRaw.lib.e_sqlite3` ≤ 2.1.11, reached transitively through EF Core's SQLite provider, bundles SQLite < 3.50.2, where aggregate terms exceeding the available columns can corrupt memory | Low | Medium | **No patched version exists** — the advisory lists *Patched versions: None*, so this cannot be fixed by upgrading. Exploitation requires submitting crafted SQL; this application issues no raw SQL, every query goes through EF Core parameterised, and the database file is not network-reachable (R11). Recorded in `known-advisories.txt` so it does not mask a *new* advisory | A patched `SQLitePCLRaw` being released, or the hosting decision keeping SQLite in production | Dai Lam La La — **review with the hosting decision** (was due 9 Sep 2026; still open) |

### Technology and legal

| # | Risk | L | I | Mitigation | Trigger | Owner |
| --- | --- | --- | --- | --- | --- | --- |
| R18 | .NET 10 immaturity — an SDK or library regression blocks the build | Low | Medium | SDK version pinned in CI; dependency surface kept minimal; no preview-only features; lockfile committed | A build failing on the toolchain rather than on our code | Chenxu You |
...
| Version | Date | Change |
| --- | --- | --- |
| 1.3 | 25 Sep 2026 | **Mitigations updated after the 25 September audit**, which found three critical gaps now fixed. **R15** gains the concurrency stamp: a double-clicked approval, or two approvers at once, could rewrite a sealed record's snapshot and hash, or return a sealed cycle to editable. **R16** records Caddy in front of the application, anti-framing headers and a per-address sign-in limit. **R14** records the bootstrap administrator and the forced password change. **R12** now points at the tests that prove it. No row is added, removed or re-scored |
| 1.2 | 16 Sep 2026 | **Owners re-mapped to the five technical layers** agreed on 15 September ([minutes](../meetings/team/2026-09-15-team-meeting.md), [`plan.md` §3](plan.md)). Calculation-method risks R2 and R3 move to Wenmin Luo, who now owns the engine's assumptions. Deployment R6, R17 and R17a move to Dai Lam La La; R16 is split between transport (Dai Lam La La) and session (Jaswanth Vericherla). Access control R12 and seeded credentials R14 move to Jaswanth Vericherla. The seal R15 and the toolchain R18 move to Chenxu You. R11 is split between the views that encode output and the page models that bind input. R16's mitigation updated to the sign-in hardened in PR #64 (2-hour session, lockout, PBKDF2, security stamp). No likelihood, impact or trigger changed. Dai Lam La La remains owner of this document |
