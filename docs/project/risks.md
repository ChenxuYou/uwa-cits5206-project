...
| R16 | Transport or session weakness | Low | Medium | HTTPS only with HSTS; HTTP-only cookie, Secure outside development, with a 2-hour expiry and sliding renewal; PBKDF2 password hashing at 210,000 iterations with rehash on sign-in; five failed attempts lock an account for 15 minutes; a security stamp rotated on password change ends older sessions — no custom cryptography | Any HTTP endpoint reachable in staging | Dai Lam La La (transport), Jaswanth Vericherla (session) |
