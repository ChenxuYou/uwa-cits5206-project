...
| R16 | Transport or session weakness | Low | Medium | HTTPS only with HSTS; cookie authentication with an 8-hour expiry and sliding renewal; framework-provided password hashing — no custom cryptography | Any HTTP endpoint reachable in staging | Dai Lam La La (transport), Jaswanth Vericherla (session) |
